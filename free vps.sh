...
EOF
    ...
    echo
    printf "\e[1;32m                  GORRILA CODERZ VM MANAGER v5.0\e[0m\n"
...
setup_vm_image() {
    ...
    if ! qemu-img resize "$IMG_FILE" "$DISK_SIZE" 2>/dev/null; then
        print_status "WARN" "Failed to resize disk image. Creating a new image with the specified size..."
        rm -f "$IMG_FILE"
    ...
    if load_vm_config "$vm_name"; then
        ...
        if [[ -n "$PORT_FORWARDS" ]]; then
            ...
            for forward in "${forwards[@]}"; do
                ...
                qemu_cmd+=(-device "virtio-net-pci,netdev=n${port_idx}")
                qemu_cmd+=(-netdev "user,id=n${port_idx},hostfwd=tcp::$host_port-:$guest_port")
                ((port_idx++))
            done
        fi
